/*  GPS_Receiver.ino  –  v2.0
 *  Pro Micro (ATmega32U4) + NRF24L01  →  HID Keyboard
 *
 *  Receives "LAT,LNG" from the ESP32 transmitter and opens the
...
/* ── Netlify base URL ─────────────────────────────────────── */
const char* NETLIFY_BASE = "https://danger-monitor-profile.netlify.app/?loc=";
...
void loop(){
  ...
  String coords = String(payload);
  coords.trim();

  /* Basic sanity check – must contain a comma separating lat/lng */
  if(coords.indexOf(',') < 0){
  ...
  Serial.print("[NRF] Received coords: ");
  Serial.println(coords);
  ...
  /* 3. Type the full Netlify alert URL with coordinates embedded */
  Keyboard.print(NETLIFY_BASE);
